...
Create Database PropertyDBContext
Go
Use [PropertyDBContext]
Go
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Negotiator]') AND [c].[name] = N'WebSite');
IF @var2 IS NOT NULL EXEC(N'ALTER TABLE [Negotiator] DROP CONSTRAINT [' + @var2 + '];');
ALTER TABLE [Negotiator] ALTER COLUMN [WebSite] nvarchar(max) NULL;

DECLARE @var3 sysname;
SELECT @var3 = [d].[name]
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Negotiator]') AND [c].[name] = N'Email');
IF @var3 IS NOT NULL EXEC(N'ALTER TABLE [Negotiator] DROP CONSTRAINT [' + @var3 + '];');
ALTER TABLE [Negotiator] ALTER COLUMN [Email] nvarchar(max) NULL;

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20241211200316_v10', N'9.0.0');
